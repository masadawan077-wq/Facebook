...
public class Main {
    ...
    public static User Input_User() {
        ...
        return new User(f_n, l_n, date, bio, credentials);
    }
            ...
            switch (Input_Int("Choice")) {
                case 1 -> {
                    ...
                    while (running) {
                        ...
                        switch (Input_Int("Choice")) {
                            case 1 -> {
                                Add_in_Feed(Database.Load_Friends(Main.current.getCredentials().getUsername()), path,
                                        post);
                                running = false;
                            }
                            case 2 -> {
    ...
    public static Notification Input_NotificationT() {
        return new Notification(Notification.Type.TAG,
                current.getCredentials().getUsername() + " Tagged you in a post");
    }
    ...
    public static Notification Input_NotificationC() {
        return new Notification(Notification.Type.COMMENT,
                current.getCredentials().getUsername() + " Commented on ur post");
    }
